<div>
    ...
    <div x-show="isOpen" x-transition @keydown.escape.window="closeModal" x-cloak
        ...
        <div @click.away="closeModal" class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6">
            ...
            <!-- Header -->
            <h2 class="text-xl font-semibold text-gray-800">New Collection</h2>
            ...
            return {
                ...
                submitForm() {
                    if (this.validate()) {
                        ...
                        fetch("{{ route('gallery-categories.store') }}", {
                                method: 'POST',
                                body: formData,
                                headers: {
                            ...
                            .then(data => {
                                if (!data.success) {
                                    this.apiError = data.message || 'Unable to create category.';
                                    return;
                                }
                                this.closeModal();
                            ...
                            .catch(err => {
                                console.error(err);
                                if (!this.apiError) {
                                    this.apiError = err.message || 'Something went wrong while creating category.';
                                }
                            });
                    }
                },

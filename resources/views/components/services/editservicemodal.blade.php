...
<div x-show="showEditModal" x-cloak @click.self="showEditModal = false"
    ...
    <div x-show="showEditModal" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="p-4 overflow-y-auto max-h-[calc(100vh-200px)]">
            <form :action="editServiceRoute" method="POST" enctype="multipart/form-data" class="space-y-4"
                x-data="{
                    selectedIcon: '',
                ...
                }" x-init="selectedIcon = editServiceData.icon || ''"
                @modal-opened.window="selectedIcon = editServiceData.icon || ''">
                @csrf
                @method('PUT')
                ...
                <div class="relative" @click.away="showIconDropdown = false">
                    ...
                    <button type="button" @click="showIconDropdown = !showIconDropdown"
                        class="w-full px-4 py-2 text-left border border-gray-300 rounded-lg
                                focus:ring-2 focus:ring-blue-500 bg-white flex items-center justify-between">
                ...
                <div>
                    <label for="edit_service_name" class="block text-sm font-medium text-gray-700 mb-2">
                        Service Name <span class="text-red-500">*</span>
                    </label>
                    <input type="text" id="edit_service_name" name="service_name" placeholder="Enter service name..."
                        x-model="editServiceData.service_name"
                ...
                <div>
                    ...
                    <textarea id="edit_description" name="description" rows="4" placeholder="Enter service description..."
                        x-model="editServiceData.description"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        required></textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
